...
void odm_ant_div_on_off(void *dm_void, u8 swch)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct phydm_fat_struct *fat_tab = &dm->dm_fat_table;
		...
		if (dm->support_ic_type & ODM_N_ANTDIV_SUPPORT) {
			...
			if (dm->support_ic_type == ODM_RTL8723D) {
				if (swch == ANTDIV_ON) {
					...
					odm_set_bb_reg(dm, R_0x948, BIT(6), 1); /*1:HW ctrl  0:SW ctrl*/
				} else {
					odm_set_bb_reg(dm, R_0xce0, BIT(1), 0);
					odm_set_bb_reg(dm, R_0x948, BIT(6), 0); /*1:HW ctrl  0:SW ctrl*/
				}
			}
#endif
			...
			if (dm->support_ic_type & ODM_RTL8812) {
				odm_set_bb_reg(dm, R_0xc50, BIT(7), swch); /* OFDM AntDiv function block enable */
				odm_set_bb_reg(dm, R_0xa00, BIT(15), swch); /* CCK AntDiv function block enable */
			} else {
				odm_set_bb_reg(dm, R_0x8d4, BIT(24), swch); /* OFDM AntDiv function block enable */
	...
	void *dm_void)
{
	...
	/* Mapping table */
	odm_set_bb_reg(dm, R_0xca4, 0xFFFF, 0x0100); /* antenna mapping table */
	...
	u32 optional_ant)
{
	...
	odm_set_bb_reg(dm, R_0x948, BIT(7), default_ant);
	odm_set_bb_reg(dm, R_0x864, BIT(5) | BIT(4) | BIT(3), default_ant); /*Default RX*/
	odm_set_bb_reg(dm, R_0x864, BIT(8) | BIT(7) | BIT(6), optional_ant); /*Optional RX*/
	odm_set_bb_reg(dm, R_0x860, BIT(14) | BIT(13) | BIT(12), default_ant); /*Default TX*/
	fat_tab->rx_idle_ant = ant;
	...
	u8 step)
{
	...
	if (dm_swat_table->try_flag == SWAW_STEP_INIT) {
		...
		if (dm_swat_table->try_flag == SWAW_STEP_PEEK) {
			...
			if (dm->support_ic_type == ODM_RTL8723D) {
				value32 = odm_get_bb_reg(dm, R_0x864, BIT(5) | BIT(4) | BIT(3));
				if ((fat_tab->rx_idle_ant == MAIN_ANT && value32 == 1) || (fat_tab->rx_idle_ant == AUX_ANT && value32 == 0))
	...
	u32 *_out_len)
{
	...
	if (dm_value[0] == 1) { /*fixed or auto antenna*/
		...
		PDM_SNPF(out_len, used, output + used, out_len - used,
			 "AntDiv_period=((%d))\n", dm->antdiv_period);
	} else if (dm_value[0] == 3) {/*Only for 8821C*/
		if (dm_value[1] == 0) {
			...
			PDM_SNPF(out_len, used, output + used, out_len - used,
				 "21C AntDiv: HW diversity\n");
		} else if (dm_value[1] == 2) {
			fat_tab->force_antdiv_type = true;
			fat_tab->antdiv_type_dbg = S0S1_SW_ANTDIV;
			PDM_SNPF(out_len, used, output + used, out_len - used,
				 "21C AntDiv: SW diversity\n");
		}
	}
	*_used = used;
	*_out_len = out_len;
