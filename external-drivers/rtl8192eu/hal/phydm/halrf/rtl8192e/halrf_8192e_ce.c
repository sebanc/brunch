...
)
{
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD, ("TxPwrTracking path %c: X = 0x%x, Y = 0x%x ele_A = 0x%x ele_C = 0x%x ele_D = 0x%x 0xeb4 = 0x%x 0xebc = 0x%x\n",
...
)
{
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD, ("===>ODM_TxPwrTrackSetPwr8192EA\n"));
	...
	if (method == TXAGC) {
		...
		ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD, ("odm_TxPwrTrackSetPwr92E CH=%d\n", *(p_dm->p_channel)));
		...
		if (*(p_dm->p_mp_mode) == true) {
			if (rf_path == RF_PATH_A) {
				...
				pwr += p_dm->rf_calibrate_info.power_index_offset[RF_PATH_A];
				...
				ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD, ("odm_tx_pwr_track_set_pwr92_e: CCK Tx-rf(A) Power = 0x%x\n", tx_agc));
				...
				ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD, ("odm_tx_pwr_track_set_pwr92_e: OFDM Tx-rf(A) Power = 0x%x\n", tx_agc));
			} else if (rf_path == RF_PATH_B) {
				/* CCK path B */
				pwr = phy_query_bb_reg(adapter, REG_TX_AGC_B_RATE18_06, 0xFF);
				pwr += p_dm->rf_calibrate_info.power_index_offset[RF_PATH_B];
				...
				ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD, ("odm_tx_pwr_track_set_pwr92_e: CCK Tx-rf(B) Power = 0x%x\n", pwr));
				...
				ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD, ("odm_tx_pwr_track_set_pwr92_e: OFDM Tx-rf(B) Power = 0x%x\n", tx_agc));
			}
		} else
#endif
		{
			/* PHY_SetTxPowerLevelByPath8192E(adapter, p_hal_data->current_channel, rf_path);   */ /* Using new set power function */
			/* phy_set_tx_power_level8192e(p_dm->adapter, *p_dm->p_channel); */
			...
			if (rf_path == RF_PATH_A) {
				phy_set_tx_power_index_by_rate_section(adapter, RF_PATH_A, *p_dm->p_channel, CCK);
				phy_set_tx_power_index_by_rate_section(adapter, RF_PATH_A, *p_dm->p_channel, OFDM);
				phy_set_tx_power_index_by_rate_section(adapter, RF_PATH_A, *p_dm->p_channel, HT_MCS0_MCS7);
				phy_set_tx_power_index_by_rate_section(adapter, RF_PATH_A, *p_dm->p_channel, HT_MCS8_MCS15);
			} else {
				phy_set_tx_power_index_by_rate_section(adapter, RF_PATH_B, *p_dm->p_channel, CCK);
				phy_set_tx_power_index_by_rate_section(adapter, RF_PATH_B, *p_dm->p_channel, OFDM);
				phy_set_tx_power_index_by_rate_section(adapter, RF_PATH_B, *p_dm->p_channel, HT_MCS0_MCS7);
				phy_set_tx_power_index_by_rate_section(adapter, RF_PATH_B, *p_dm->p_channel, HT_MCS8_MCS15);
			}
		}
		...
		if (rf_path == RF_PATH_A) {
			set_iqk_matrix_8192e(p_dm, final_ofdm_swing_index, RF_PATH_A,
			...
			if (!p_dm->rf_calibrate_info.is_cck_in_ch14) {
				odm_write_1byte(p_dm, 0xa22, cck_swing_table_ch1_ch13_new[final_cck_swing_index][0]);
				odm_write_1byte(p_dm, 0xa23, cck_swing_table_ch1_ch13_new[final_cck_swing_index][1]);
				odm_write_1byte(p_dm, 0xa24, cck_swing_table_ch1_ch13_new[final_cck_swing_index][2]);
				odm_write_1byte(p_dm, 0xa25, cck_swing_table_ch1_ch13_new[final_cck_swing_index][3]);
				odm_write_1byte(p_dm, 0xa26, cck_swing_table_ch1_ch13_new[final_cck_swing_index][4]);
				odm_write_1byte(p_dm, 0xa27, cck_swing_table_ch1_ch13_new[final_cck_swing_index][5]);
				odm_write_1byte(p_dm, 0xa28, cck_swing_table_ch1_ch13_new[final_cck_swing_index][6]);
				odm_write_1byte(p_dm, 0xa29, cck_swing_table_ch1_ch13_new[final_cck_swing_index][7]);
			} else {
				odm_write_1byte(p_dm, 0xa22, cck_swing_table_ch14_new[final_cck_swing_index][0]);
				odm_write_1byte(p_dm, 0xa23, cck_swing_table_ch14_new[final_cck_swing_index][1]);
				odm_write_1byte(p_dm, 0xa24, cck_swing_table_ch14_new[final_cck_swing_index][2]);
				odm_write_1byte(p_dm, 0xa25, cck_swing_table_ch14_new[final_cck_swing_index][3]);
				odm_write_1byte(p_dm, 0xa26, cck_swing_table_ch14_new[final_cck_swing_index][4]);
				odm_write_1byte(p_dm, 0xa27, cck_swing_table_ch14_new[final_cck_swing_index][5]);
				odm_write_1byte(p_dm, 0xa28, cck_swing_table_ch14_new[final_cck_swing_index][6]);
				odm_write_1byte(p_dm, 0xa29, cck_swing_table_ch14_new[final_cck_swing_index][7]);
			}
		} else if (rf_path == RF_PATH_B) {
			set_iqk_matrix_8192e(p_dm, final_ofdm_swing_index, RF_PATH_B,
				p_dm->rf_calibrate_info.iqk_matrix_reg_setting[channel_mapped_index].value[0][4],
				p_dm->rf_calibrate_info.iqk_matrix_reg_setting[channel_mapped_index].value[0][5]);
		}
	} else if (method == MIX_MODE) {
		ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD, ("p_rf_calibrate_info->default_ofdm_index=%d,  p_dm->DefaultCCKIndex=%d, p_rf_calibrate_info->absolute_ofdm_swing_idx[rf_path]=%d, rf_path = %d\n",
		...
		if (rf_path == RF_PATH_A) {
			...
			if (final_ofdm_swing_index > pwr_tracking_limit_ofdm) {   /*BBSwing higher then Limit*/
				...
				set_iqk_matrix_8192e(p_dm, pwr_tracking_limit_ofdm, RF_PATH_A,
					p_dm->rf_calibrate_info.iqk_matrix_reg_setting[channel_mapped_index].value[0][0],
					p_dm->rf_calibrate_info.iqk_matrix_reg_setting[channel_mapped_index].value[0][1]);
				...
				set_iqk_matrix_8192e(p_dm, 0, RF_PATH_A,
					p_dm->rf_calibrate_info.iqk_matrix_reg_setting[channel_mapped_index].value[0][0],
					p_dm->rf_calibrate_info.iqk_matrix_reg_setting[channel_mapped_index].value[0][1]);
			...
			if (final_cck_swing_index > pwr_tracking_limit_cck) {
				...
				ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD, ("******Path_A CCK Over Limit, pwr_tracking_limit_cck = %d, p_rf_calibrate_info->remnant_cck_swing_idx  = %d\n", pwr_tracking_limit_cck, p_rf_calibrate_info->remnant_cck_swing_idx));

				/* Adjust BB swing by CCK filter coefficient*/

				if (!p_dm->rf_calibrate_info.is_cck_in_ch14) {
					odm_write_1byte(p_dm, 0xa22, cck_swing_table_ch1_ch13_new[pwr_tracking_limit_cck][0]);
					odm_write_1byte(p_dm, 0xa23, cck_swing_table_ch1_ch13_new[pwr_tracking_limit_cck][1]);
					odm_write_1byte(p_dm, 0xa24, cck_swing_table_ch1_ch13_new[pwr_tracking_limit_cck][2]);
					odm_write_1byte(p_dm, 0xa25, cck_swing_table_ch1_ch13_new[pwr_tracking_limit_cck][3]);
					odm_write_1byte(p_dm, 0xa26, cck_swing_table_ch1_ch13_new[pwr_tracking_limit_cck][4]);
					odm_write_1byte(p_dm, 0xa27, cck_swing_table_ch1_ch13_new[pwr_tracking_limit_cck][5]);
					odm_write_1byte(p_dm, 0xa28, cck_swing_table_ch1_ch13_new[pwr_tracking_limit_cck][6]);
					odm_write_1byte(p_dm, 0xa29, cck_swing_table_ch1_ch13_new[pwr_tracking_limit_cck][7]);
				} else {
					odm_write_1byte(p_dm, 0xa22, cck_swing_table_ch14_new[pwr_tracking_limit_cck][0]);
					odm_write_1byte(p_dm, 0xa23, cck_swing_table_ch14_new[pwr_tracking_limit_cck][1]);
					odm_write_1byte(p_dm, 0xa24, cck_swing_table_ch14_new[pwr_tracking_limit_cck][2]);
					odm_write_1byte(p_dm, 0xa25, cck_swing_table_ch14_new[pwr_tracking_limit_cck][3]);
					odm_write_1byte(p_dm, 0xa26, cck_swing_table_ch14_new[pwr_tracking_limit_cck][4]);
					odm_write_1byte(p_dm, 0xa27, cck_swing_table_ch14_new[pwr_tracking_limit_cck][5]);
					odm_write_1byte(p_dm, 0xa28, cck_swing_table_ch14_new[pwr_tracking_limit_cck][6]);
					odm_write_1byte(p_dm, 0xa29, cck_swing_table_ch14_new[pwr_tracking_limit_cck][7]);
				}
				...
				ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD, ("******Path_A CCK Under Limit, pwr_tracking_limit_cck = %d, p_rf_calibrate_info->remnant_cck_swing_idx  = %d\n", 0, p_rf_calibrate_info->remnant_cck_swing_idx));

				if (!p_dm->rf_calibrate_info.is_cck_in_ch14) {
					odm_write_1byte(p_dm, 0xa22, cck_swing_table_ch1_ch13_new[0][0]);
					odm_write_1byte(p_dm, 0xa23, cck_swing_table_ch1_ch13_new[0][1]);
					odm_write_1byte(p_dm, 0xa24, cck_swing_table_ch1_ch13_new[0][2]);
					odm_write_1byte(p_dm, 0xa25, cck_swing_table_ch1_ch13_new[0][3]);
					odm_write_1byte(p_dm, 0xa26, cck_swing_table_ch1_ch13_new[0][4]);
					odm_write_1byte(p_dm, 0xa27, cck_swing_table_ch1_ch13_new[0][5]);
					odm_write_1byte(p_dm, 0xa28, cck_swing_table_ch1_ch13_new[0][6]);
					odm_write_1byte(p_dm, 0xa29, cck_swing_table_ch1_ch13_new[0][7]);
				} else {
					odm_write_1byte(p_dm, 0xa22, cck_swing_table_ch14_new[0][0]);
					odm_write_1byte(p_dm, 0xa23, cck_swing_table_ch14_new[0][1]);
					odm_write_1byte(p_dm, 0xa24, cck_swing_table_ch14_new[0][2]);
					odm_write_1byte(p_dm, 0xa25, cck_swing_table_ch14_new[0][3]);
					odm_write_1byte(p_dm, 0xa26, cck_swing_table_ch14_new[0][4]);
					odm_write_1byte(p_dm, 0xa27, cck_swing_table_ch14_new[0][5]);
					odm_write_1byte(p_dm, 0xa28, cck_swing_table_ch14_new[0][6]);
					odm_write_1byte(p_dm, 0xa29, cck_swing_table_ch14_new[0][7]);
				}
		...
		if (rf_path == RF_PATH_B) {
			if (final_ofdm_swing_index > pwr_tracking_limit_ofdm) {  /*BBSwing higher then Limit*/
				...
				set_iqk_matrix_8192e(p_dm, pwr_tracking_limit_ofdm, RF_PATH_B,
					p_dm->rf_calibrate_info.iqk_matrix_reg_setting[channel_mapped_index].value[0][4],
					p_dm->rf_calibrate_info.iqk_matrix_reg_setting[channel_mapped_index].value[0][5]);
				...
				set_iqk_matrix_8192e(p_dm, 0, RF_PATH_B,
					p_dm->rf_calibrate_info.iqk_matrix_reg_setting[channel_mapped_index].value[0][4],
					p_dm->rf_calibrate_info.iqk_matrix_reg_setting[channel_mapped_index].value[0][5]);
...
)
{
	...
	odm_set_rf_reg(p_dm, RF_PATH_A, 0xdf, RFREGOFFSETMASK, 0x180);
	...
	odm_set_rf_reg(p_dm, RF_PATH_A, RF_TXPA_G2, RFREGOFFSETMASK, 0x07f77);  /* PA off, default: 0x7f7f */
	...
	odm_set_bb_reg(p_dm, REG_TX_IQK_PI_A, MASKDWORD, 0x82140303);
	odm_set_bb_reg(p_dm, REG_RX_IQK_PI_A, MASKDWORD, 0x68160000);
	...
	* 	ODM_RT_TRACE(p_dm,ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("delay %d ms for One shot, path A LOK & IQK.\n", IQK_DELAY_TIME_92E)); */
	...
	if (!(reg_eac & BIT(28)) &&
		...
		ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("pathA TX IQK is not success\n"));
	return result;
}
...
)
{
	...
	/* 1 Get TXIMR setting */
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("Get RXIQK TXIMR!\n"));
	...
	* 	ODM_RT_TRACE(p_dm,ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("path-A Rx IQK modify RXIQK mode table!\n")); */
	...
	odm_set_rf_reg(p_dm, RF_PATH_A, RF_TXPA_G1, RFREGOFFSETMASK, 0x0000f);
	odm_set_rf_reg(p_dm, RF_PATH_A, RF_TXPA_G2, RFREGOFFSETMASK, 0xf1173); /* PA off, deafault:0xf117b */

	odm_set_rf_reg(p_dm, RF_PATH_B, RF_WE_LUT, RFREGOFFSETMASK, 0x800a0);
	odm_set_rf_reg(p_dm, RF_PATH_B, RF_RCK_OS, RFREGOFFSETMASK, 0x30000);
	odm_set_rf_reg(p_dm, RF_PATH_B, RF_TXPA_G1, RFREGOFFSETMASK, 0x0000f);
	odm_set_rf_reg(p_dm, RF_PATH_B, RF_TXPA_G2, RFREGOFFSETMASK, 0xf1173); /* PA off, deafault:0xf117b */

	/*	PA/PAD control by 0x56, and set = 0x0 */
	odm_set_rf_reg(p_dm, RF_PATH_A, 0xdf, RFREGOFFSETMASK, 0x980);
	odm_set_rf_reg(p_dm, RF_PATH_A, 0x56, RFREGOFFSETMASK, 0x511e0);
	...
	odm_set_bb_reg(p_dm, REG_TX_IQK_PI_A, MASKDWORD, 0x8216031f);
	odm_set_bb_reg(p_dm, REG_RX_IQK_PI_A, MASKDWORD, 0x6816031f);
	...
	* 	ODM_RT_TRACE(p_dm,ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("delay %d ms for One shot, path A LOK & IQK.\n", IQK_DELAY_TIME_92E)); */
	...
	if (!(reg_eac & BIT(28)) &&
		...
		odm_set_bb_reg(p_dm, REG_FPGA0_IQK, 0xffffff00, 0x000000);
		odm_set_rf_reg(p_dm, RF_PATH_A, 0xdf, RFREGOFFSETMASK, 0x180);
		ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("pathA get TXIMR is not success\n"));
		return result;
	...
	/* 1 RX IQK */
	/* modify RXIQK mode table */
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("Do RXIQK!\n"));
	...
	odm_set_rf_reg(p_dm, RF_PATH_A, RF_RCK_OS, RFREGOFFSETMASK, 0x30000);
	odm_set_rf_reg(p_dm, RF_PATH_A, RF_TXPA_G1, RFREGOFFSETMASK, 0x0000f);
	odm_set_rf_reg(p_dm, RF_PATH_A, RF_TXPA_G2, RFREGOFFSETMASK, 0xf7ff2);  /*PA off : default:0xf7ffa*/

	odm_set_rf_reg(p_dm, RF_PATH_B, RF_WE_LUT, RFREGOFFSETMASK, 0x800a0);
	odm_set_rf_reg(p_dm, RF_PATH_B, RF_RCK_OS, RFREGOFFSETMASK, 0x30000);
	odm_set_rf_reg(p_dm, RF_PATH_B, RF_TXPA_G1, RFREGOFFSETMASK, 0x0000f);
	odm_set_rf_reg(p_dm, RF_PATH_B, RF_TXPA_G2, RFREGOFFSETMASK, 0xf7ff2);  /*PA off : default:0xf7ffa*/


	/*	PA/PAD control by 0x56, and set = 0x0 */
	odm_set_rf_reg(p_dm, RF_PATH_A, 0xdf, RFREGOFFSETMASK, 0x980);
	odm_set_rf_reg(p_dm, RF_PATH_A, 0x56, RFREGOFFSETMASK, 0x510e0);
	...
	odm_set_bb_reg(p_dm, REG_TX_IQK_PI_A, MASKDWORD, 0x821608ff);
	odm_set_bb_reg(p_dm, REG_RX_IQK_PI_A, MASKDWORD, 0x281608ff);

	/*	odm_set_bb_reg(p_dm, REG_TX_IQK_PI_A, MASKDWORD, 0x82160cff);
	 *	odm_set_bb_reg(p_dm, REG_RX_IQK_PI_A, MASKDWORD, 0x28160cff); */

	/* LO calibration setting
	* 	ODM_RT_TRACE(p_dm,ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("LO calibration setting!\n")); */
	odm_set_bb_reg(p_dm, REG_IQK_AGC_RSP, MASKDWORD, 0x0046a891);
	...
	* 	ODM_RT_TRACE(p_dm,ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("delay %d ms for One shot, path A LOK & IQK.\n", IQK_DELAY_TIME_92E)); */
	...
	if (!(reg_eac & BIT(27)) &&	/*if Tx is OK, check whether Rx is OK*/
		...
		ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("path A Rx IQK is not success!!\n"));
...
)
{
	...
	* 	ODM_RT_TRACE(p_dm,ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("path-B IQK setting!\n")); */
	...
	odm_set_bb_reg(p_dm, 0x820, BIT(8), 0x0);
	...
	/* modify TXIQK mode table */
	odm_set_rf_reg(p_dm, RF_PATH_B, RF_WE_LUT, RFREGOFFSETMASK, 0x800a0);
	odm_set_rf_reg(p_dm, RF_PATH_B, RF_RCK_OS, RFREGOFFSETMASK, 0x20000);
	odm_set_rf_reg(p_dm, RF_PATH_B, RF_TXPA_G1, RFREGOFFSETMASK, 0x0000f);
	odm_set_rf_reg(p_dm, RF_PATH_B, RF_TXPA_G2, RFREGOFFSETMASK, 0x07f77);  /* PA off, default: 0x7f7f */

	odm_set_bb_reg(p_dm, REG_FPGA0_IQK, 0xffffff00, 0x808000);
#if 0
	odm_set_bb_reg(p_dm, 0xe28, 0xffffff00, 0x000000);
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("path A 0xdf = 0x%x\n", odm_get_rf_reg(p_dm, RF_PATH_A, 0xdf, RFREGOFFSETMASK)));
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("path B 0xdf = 0x%x\n", odm_get_rf_reg(p_dm, RF_PATH_B, 0xdf, RFREGOFFSETMASK)));
	odm_set_bb_reg(p_dm, 0xe28, 0xffffff00, 0x808000);
#endif

	odm_set_bb_reg(p_dm, REG_TX_IQK_TONE_A, MASKDWORD, 0x38008c1c);
	odm_set_bb_reg(p_dm, REG_RX_IQK_TONE_A, MASKDWORD, 0x38008c1c);
	odm_set_bb_reg(p_dm, REG_TX_IQK_TONE_B, MASKDWORD, 0x18008c1c);
	odm_set_bb_reg(p_dm, REG_RX_IQK_TONE_B, MASKDWORD, 0x38008c1c);

	odm_set_bb_reg(p_dm, REG_TX_IQK_PI_B, MASKDWORD, 0x82140303);
	odm_set_bb_reg(p_dm, REG_RX_IQK_PI_B, MASKDWORD, 0x68160000);
	...
	* 	ODM_RT_TRACE(p_dm,ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("delay %d ms for One shot, path B LOK & IQK.\n", IQK_DELAY_TIME_92E)); */
	...
	/*enable path-A PI*/
	odm_set_bb_reg(p_dm, 0x820, BIT(8), 0x1);
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("0xeac = 0x%x\n", reg_eac));
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("0xeb4 = 0x%x, 0xebc = 0x%x\n", reg_eb4, reg_ebc));
	/*monitor image power before & after IQK*/
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("0xeb0(before IQK)= 0x%x, 0xeb8(afer IQK) = 0x%x\n",
		odm_get_bb_reg(p_dm, 0xeb0, MASKDWORD), odm_get_bb_reg(p_dm, 0xeb8, MASKDWORD)));
	...
	if (!(reg_eac & BIT(31)) &&
		...
		ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("path B TX IQK is not success\n"));
	return result;
}
...
)
{
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("Get RXIQK TXIMR!\n"));
	/* modify RXIQK mode table
	* 	ODM_RT_TRACE(p_dm,ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("path-B Rx IQK modify RXIQK mode table!\n")); */
	odm_set_bb_reg(p_dm, REG_FPGA0_IQK, 0xffffff00, 0x000000);

	odm_set_rf_reg(p_dm, RF_PATH_B, RF_WE_LUT, RFREGOFFSETMASK, 0x800a0);
	odm_set_rf_reg(p_dm, RF_PATH_B, RF_RCK_OS, RFREGOFFSETMASK, 0x30000);
	odm_set_rf_reg(p_dm, RF_PATH_B, RF_TXPA_G1, RFREGOFFSETMASK, 0x0000f);
	odm_set_rf_reg(p_dm, RF_PATH_B, RF_TXPA_G2, RFREGOFFSETMASK, 0xf1173);  /*PA off, default: 0xf117b*/

	odm_set_rf_reg(p_dm, RF_PATH_A, RF_WE_LUT, RFREGOFFSETMASK, 0x800a0);
	odm_set_rf_reg(p_dm, RF_PATH_A, RF_RCK_OS, RFREGOFFSETMASK, 0x30000);
	odm_set_rf_reg(p_dm, RF_PATH_A, RF_TXPA_G1, RFREGOFFSETMASK, 0x0000f);
	odm_set_rf_reg(p_dm, RF_PATH_A, RF_TXPA_G2, RFREGOFFSETMASK, 0xf1173);  /*PA off, default: 0xf117b*/

	/*	PA/PAD all off */
	odm_set_rf_reg(p_dm, RF_PATH_B, 0xdf, RFREGOFFSETMASK, 0x980);
	odm_set_rf_reg(p_dm, RF_PATH_B, 0x56, RFREGOFFSETMASK, 0x511e0);
	...
	odm_set_bb_reg(p_dm, REG_TX_IQK_PI_B, MASKDWORD, 0x8216031f);
	odm_set_bb_reg(p_dm, REG_RX_IQK_PI_B, MASKDWORD, 0x6816031f);
	...
	* 	ODM_RT_TRACE(p_dm,ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("delay %d ms for One shot, path B LOK & IQK.\n", IQK_DELAY_TIME_92E)); */
	...
	if (!(reg_eac & BIT(31)) &&
		...
		odm_set_bb_reg(p_dm, REG_FPGA0_IQK, 0xffffff00, 0x000000);
		odm_set_rf_reg(p_dm, RF_PATH_B, 0xdf, RFREGOFFSETMASK, 0x180);
		ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("pathB get TXIMR is not success\n"));
		return result;
	}
	...
	/* modify RXIQK mode table
	* 	ODM_RT_TRACE(p_dm,ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("path-B Rx IQK modify RXIQK mode table 2!\n")); */
	odm_set_bb_reg(p_dm, REG_FPGA0_IQK, 0xffffff00, 0x000000);
	...
	/*	PA/PAD all off */
	odm_set_rf_reg(p_dm, RF_PATH_B, 0xdf, RFREGOFFSETMASK, 0x980);
	odm_set_rf_reg(p_dm, RF_PATH_B, 0x56, RFREGOFFSETMASK, 0x510e0);
	/*	odm_set_rf_reg(p_dm, RF_PATH_B, 0x56, RFREGOFFSETMASK, 0x51000 ); */
	...
	odm_set_bb_reg(p_dm, REG_RX_IQK_TONE_A, MASKDWORD, 0x38008c1c);
	odm_set_bb_reg(p_dm, REG_TX_IQK_TONE_B, MASKDWORD, 0x38008c1c);
	odm_set_bb_reg(p_dm, REG_RX_IQK_TONE_B, MASKDWORD, 0x18008c1c);

	odm_set_bb_reg(p_dm, REG_TX_IQK_PI_B, MASKDWORD, 0x821608ff);
	odm_set_bb_reg(p_dm, REG_RX_IQK_PI_B, MASKDWORD, 0x281608ff);

	/*	odm_set_bb_reg(p_dm, REG_TX_IQK_PI_B, MASKDWORD, 0x82160cff);
	 *	odm_set_bb_reg(p_dm, REG_RX_IQK_PI_B, MASKDWORD, 0x28160cff); */
	...
	* 	ODM_RT_TRACE(p_dm,ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("delay %d ms for One shot, path B LOK & IQK.\n", IQK_DELAY_TIME_92E)); */
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("0xeac = 0x%x\n", reg_eac));
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("0xec4 = 0x%x, 0xecc = 0x%x\n", reg_ec4, reg_ecc));
	/* monitor image power before & after IQK */
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("0xec0(before IQK)= 0x%x, 0xec8(afer IQK) = 0x%x\n",
		odm_get_bb_reg(p_dm, 0xec0, MASKDWORD), odm_get_bb_reg(p_dm, 0xec8, MASKDWORD)));
	...
	if (!(reg_eac & BIT(30)) &&/*if Tx is OK, check whether Rx is OK*/
		...
		ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("path B Rx IQK is not success!!\n"));
	return result;
}
...
)
{
	...
	if (final_candidate == 0xFF)
		...
		if (is_tx_only) {
			ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("_phy_path_a_fill_iqk_matrix_92e only Tx OK\n"));
			return;
		}
...
#if 0
	odm_set_bb_reg(p_dm, 0x520, MASKBYTE2, 0x0);
#else
	for (i = 0 ; i < (IQK_MAC_REG_NUM - 1); i++)
...
)
{
	...
	if (false == is2T) {
		path_on = 0x0fc01616;
		odm_set_bb_reg(p_dm, adda_reg[0], MASKDWORD, 0x0fc01616);
	} else
...
)
{
	...
	*/
	...
	/*	odm_set_bb_reg(p_dm, 0x550, 0x0000ffff, 0x0015); */
...
)
{
	...
	if (t == 0) {
		...
		_phy_save_adda_registers_92e(p_dm, ADDA_REG, p_dm->rf_calibrate_info.ADDA_backup, IQK_ADDA_REG_NUM);
		_phy_save_mac_registers_92e(p_dm, IQK_MAC_REG, p_dm->rf_calibrate_info.IQK_MAC_backup);
		_phy_save_adda_registers_92e(p_dm, IQK_BB_REG_92C, p_dm->rf_calibrate_info.IQK_BB_backup, IQK_BB_REG_NUM);
	}
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("IQ Calibration for %s for %d times\n", (is2T ? "2T2R" : "1T1R"), t));

	_phy_path_adda_on_92e(p_dm, ADDA_REG, true, is2T);
...
#else
		_phy_pi_mode_switch_92e(p_dm, true);
#endif
	}
#endif
	/* MAC settings */
	_phy_mac_setting_calibration_92e(p_dm, IQK_MAC_REG, p_dm->rf_calibrate_info.IQK_MAC_backup);
	...
	odm_set_bb_reg(p_dm, REG_FPGA0_XCD_RF_INTERFACE_SW, MASKDWORD, 0x55204200);

	if ((p_dm->ext_lna) && !(p_dm->ext_pa)) { /* external LNA / external PA = 1 /0 */
		/*PAPE force to high*/
		/*just for high power with external LNA, without external PA*/
		odm_set_bb_reg(p_dm, 0x930, 0xf, 0x7);
		odm_set_bb_reg(p_dm, 0x930, 0x0f000000, 0x7);
		odm_set_bb_reg(p_dm, 0x938, 0xf, 0x7);
		odm_set_bb_reg(p_dm, 0x938, 0x0f000000, 0x7);
		odm_set_bb_reg(p_dm, 0x92c, MASKDWORD, 0x00410041);
	} else if (p_dm->ext_pa) { /* external PA = 1*/
		/*PAPE force to low*/
		...
		odm_set_bb_reg(p_dm, 0x938, 0xf, 0x7);
		odm_set_bb_reg(p_dm, 0x938, 0x0f000000, 0x7);
		odm_set_bb_reg(p_dm, 0x92c, MASKDWORD, 0x00000000);
	}
	...
	*ODM_RT_TRACE(p_dm,ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("IQK setting!\n"));*/
	...
	if (is2T) {
		_phy_path_b_stand_by_92e(p_dm);
		/* Turn ADDA on */
		_phy_path_adda_on_92e(p_dm, ADDA_REG, false, is2T);
	}

	/* path A TXIQK */
#if 1
	for (i = 0 ; i < retry_count ; i++) {
		path_aok = phy_path_a_iqk_8192e(p_dm, is2T);
		/*		if(path_aok == 0x03){ */
		if (path_aok == 0x01) {
			...
			ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("path A Tx IQK Fail!!\n"));
#if 0
...
#if 1
	for (i = 0 ; i < retry_count ; i++) {
		path_aok = phy_path_a_rx_iqk_92e(p_dm, is2T);
		if (path_aok == 0x03) {
...
#if 1
		for (i = 0 ; i < retry_count ; i++) {
			path_bok = phy_path_b_iqk_8192e(p_dm);
			/*		if(path_bok == 0x03){ */
			if (path_bok == 0x01) {
				ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("path B Tx IQK Success!!\n"));
				result[t][4] = (odm_get_bb_reg(p_dm, REG_TX_POWER_BEFORE_IQK_B, MASKDWORD) & 0x3FF0000) >> 16;
				result[t][5] = (odm_get_bb_reg(p_dm, REG_TX_POWER_AFTER_IQK_B, MASKDWORD) & 0x3FF0000) >> 16;
				break;
			}
#if 0
			else if (i == (retry_count - 1) && path_aok == 0x01) {	/*Tx IQK OK*/
...
#if 1
		for (i = 0 ; i < retry_count ; i++) {
			path_bok = phy_path_b_rx_iqk_92e(p_dm, is2T);
			if (path_bok == 0x03) {
				ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("path B Rx IQK Success!!\n"));
				/*				result[t][0] = (odm_get_bb_reg(p_dm, REG_TX_POWER_BEFORE_IQK_A, MASKDWORD)&0x3FF0000)>>16;
				 *				result[t][1] = (odm_get_bb_reg(p_dm, REG_TX_POWER_AFTER_IQK_A, MASKDWORD)&0x3FF0000)>>16; */
				result[t][6] = (odm_get_bb_reg(p_dm, REG_RX_POWER_BEFORE_IQK_B_2, MASKDWORD) & 0x3FF0000) >> 16;
				result[t][7] = (odm_get_bb_reg(p_dm, REG_RX_POWER_AFTER_IQK_B_2, MASKDWORD) & 0x3FF0000) >> 16;
				break;
			}
		}
		if (0x00 == path_bok) {
	...
	if (t != 0) {
		...
		_phy_reload_adda_registers_92e(p_dm, ADDA_REG, p_dm->rf_calibrate_info.ADDA_backup, IQK_ADDA_REG_NUM);
		/* Reload MAC parameters */
		_phy_reload_mac_registers_92e(p_dm, IQK_MAC_REG, p_dm->rf_calibrate_info.IQK_MAC_backup);
		_phy_reload_adda_registers_92e(p_dm, IQK_BB_REG_92C, p_dm->rf_calibrate_info.IQK_BB_backup, IQK_BB_REG_NUM);
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("_phy_iq_calibrate_8192e() <==\n"));
}
...
)
{
	...
	bb_clk = odm_read_1byte(p_dm, 0xce7);
	odm_set_bb_reg(p_dm, 0xce4, 0xc0000000, 0x0);
	/* 4. Set LC calibration begin	bit15 */
	odm_set_rf_reg(p_dm, RF_PATH_A, RF_CHNLBW, MASK12BITS, lc_cal | 0x08000);
	ODM_delay_ms(100);
	/* Restore original situation */
	odm_write_1byte(p_dm, 0xce7, bb_clk);
	if ((tmp_reg & 0x70) != 0) {	/*Deal with contisuous TX case*/
		/* path-A */
...
)
{
	...
	if (is_recovery) {
		ODM_RT_TRACE(p_dm, ODM_COMP_INIT, ODM_DBG_LOUD, ("PHY_IQCalibrate_92E: Return due to is_recovery!\n"));
		_phy_reload_adda_registers_92e(p_dm, IQK_BB_REG_92C, p_dm->rf_calibrate_info.IQK_BB_backup_recover, 9);
		return;
	}
	...
	for (i = 0; i < 3; i++) {
		/*	 		_phy_iq_calibrate_8192e(p_dm, result, i, false); */
		_phy_iq_calibrate_8192e(p_dm, result, i, true);

		if (i == 1) {
			is12simular = phy_simularity_compare_8192e(p_dm, result, 0, 1);
			if (is12simular) {
				final_candidate = 0;
		...
		if (i == 2) {
			is13simular = phy_simularity_compare_8192e(p_dm, result, 0, 2);
			if (is13simular) {
				final_candidate = 0;
			...
			is23simular = phy_simularity_compare_8192e(p_dm, result, 1, 2);
			if (is23simular) {
				final_candidate = 1;
	...
	if (final_candidate != 0xff) {
		...
		ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("IQK: TX0_X=%x TX0_Y=%x RX0_X=%x RX0_Y=%x TX1_X=%x TX1_Y=%x RX1_X=%x RX1_Y=%x\n ", rege94, rege9c, regea4, regeac, regeb4, regebc, regec4, regecc));
	...
	if ((rege94 != 0)/*&&(regea4 != 0)*/) {
		_phy_path_a_fill_iqk_matrix_92e(p_dm, is_patha_ok, result, final_candidate, (regea4 == 0));
		_phy_path_b_fill_iqk_matrix_92e(p_dm, is_pathb_ok, result, final_candidate, (regec4 == 0));
	}
	...
	_phy_save_adda_registers_92e(p_dm, IQK_BB_REG_92C, p_dm->rf_calibrate_info.IQK_BB_backup_recover, IQK_BB_REG_NUM);
...
)
{
	...
	/* MAIN: WiFi */
	u32 reg = odm_get_mac_reg(p_dm, 0x64, MASKDWORD);
	reg &= 0xFFFFF000;
	reg = (is_main) ? (reg | 0x00000100) : (reg | 0x00000004);
	odm_set_mac_reg(p_dm, 0x64, MASKDWORD, reg);
...
)
{
	...
	if (odm_get_mac_reg(p_dm, 0x64, 0xFFF) == 0x100)
		return true;
	else
		return false;
...
#else
		return _phy_query_rf_path_switch_8192e(p_dm, false);
#endif
	}
}
#endif
