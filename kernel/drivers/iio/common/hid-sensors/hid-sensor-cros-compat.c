	...
	case IIO_CHAN_INFO_RAW:
		...
		if (report_id >= 0) {
			*val = sensor_hub_input_attr_get_raw_value(
					...
					report_id,
					SENSOR_HUB_SYNC,
					min < 0) * (s64)1000000000 / (als_state->scale_pre_factor * (s64)1000000000 + als_state->scale_post_factor);
		} else {
			*val = 0;
			return -EINVAL;
		}
				...
				void *priv)
{
	...
	struct als_state *als_state = iio_priv(indio_dev);
	int ret = -EINVAL;
	u16 sample_data = (u16)(((*(u32 *)(raw_data)) * (s64)1000000000) / (als_state->scale_pre_factor * (s64)1000000000 + als_state->scale_post_factor));
				...
				struct als_state *st)
{
	...
	st->scale_pre_factor = 1000000000 / (st->scale_pre_decml * (s64)1000000000 + st->scale_post_decml);
	st->scale_post_factor = (st->scale_pre_decml * (s64)1000000000 + st->scale_post_decml) - (1000000000 / st->scale_pre_factor);
	...
	case IIO_CHAN_INFO_RAW:
		...
		if (report_id >= 0)
			*val = -sensor_hub_input_attr_get_raw_value(
					...
					SENSOR_HUB_SYNC,
					min < 0) * (s64)1000000000 / (accel_state->scale_pre_factor * (s64)1000000000 + accel_state->scale_post_factor);
		else {
			*val = 0;
			return -EINVAL;
		}
				...
				void *priv)
{
	...
	case HID_USAGE_SENSOR_ACCEL_Z_AXIS:
		...
		accel_state->accel_val[CHANNEL_SCAN_INDEX_X + offset] =
			(u16)((-(*(s32 *)(raw_data)) * (s64)1000000000) / (accel_state->scale_pre_factor * (s64)1000000000 + accel_state->scale_post_factor));
		ret = 0;
	break;
				...
				struct accel_3d_state *st)
{
	...
	st->scale_pre_factor = 980665 / (st->scale_pre_decml * (s64)1000000000 + st->scale_post_decml);
	st->scale_post_factor = (st->scale_pre_decml * (s64)1000000000 + st->scale_post_decml) - (980665 / st->scale_pre_factor);
