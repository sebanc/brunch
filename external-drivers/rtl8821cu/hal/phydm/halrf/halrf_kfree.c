				 ...
				 u8 data)
{
	...
	if (wlg_btg) {
		...
		odm_set_rf_reg(dm, e_rf_path, RF_0x55, (BIT(18) | BIT(17) | BIT(16) | BIT(15) | BIT(14)), (wlg >> 1));

		odm_set_rf_reg(dm, e_rf_path, RF_0x65, BIT(19), (btg & BIT(0)));
		odm_set_rf_reg(dm, e_rf_path, RF_0x65, (BIT(18) | BIT(17) | BIT(16) | BIT(15) | BIT(14)), (btg >> 1));
	} else {
	...
	RF_DBG(dm, DBG_RF_IQK,
	       ...
	       odm_get_rf_reg(dm, e_rf_path, RF_0x65,
	       (BIT(19) | BIT(18) | BIT(17) | BIT(16) | BIT(15) | BIT(14))));
}
...
void phydm_clear_kfree_to_rf_8821c(void *dm_void, u8 e_rf_path, u8 data)
{
	...
	odm_set_rf_reg(dm, e_rf_path, RF_0x55, BIT(19), (data & BIT(0)));
	odm_set_rf_reg(dm, e_rf_path, RF_0x55, (BIT(18) | BIT(17) | BIT(16) | BIT(15) | BIT(14)), (data >> 1));

	odm_set_rf_reg(dm, e_rf_path, RF_0x65, BIT(19), (data & BIT(0)));
	odm_set_rf_reg(dm, e_rf_path, RF_0x65, (BIT(18) | BIT(17) | BIT(16) | BIT(15) | BIT(14)), (data >> 1));
...
void phydm_get_thermal_trim_offset_8822b(void *dm_void)
{
	...
	RF_DBG(dm, DBG_RF_MP, "[kfree] 8822b 0xc8:0x%2x\n",
	       thermal_trim_enable);

	thermal_trim_enable = (thermal_trim_enable & BIT(5)) >> 5;

	RF_DBG(dm, DBG_RF_MP, "[kfree] 8822b thermal trim Enable:%d\n",
	       thermal_trim_enable);
...
void phydm_get_power_trim_offset_8822b(void *dm_void)
{
	...
	RF_DBG(dm, DBG_RF_MP, "[kfree] 8822b power trim Enable:%d\n",
	       power_trim_enable);
...
void phydm_set_kfree_to_rf_8710b(void *dm_void, u8 e_rf_path, u8 data)
{
	...
	odm_set_rf_reg(dm, e_rf_path, RF_0x55, BIT(19), (data & BIT(0)));
	odm_set_rf_reg(dm, e_rf_path, RF_0x55, (BIT(18) | BIT(17) | BIT(16) | BIT(15)), ((data & 0xf) >> 1));
...
void phydm_get_power_trim_offset_8192f(void *dm_void)
{
	...
	if (power_trim_info->flag & KFREE_FLAG_ON) {
		for (i = 0; i < KFREE_CH_NUM; i++) {
			for (j = 0; j < 2; j++)
				RF_DBG(dm, DBG_RF_MP,
				       "[kfree] 8192F power_trim_data->bb_gain[%d][%d]=0x%X\n",
				       i, j, power_trim_info->bb_gain[i][j]);
		}
	}
}
				 ...
				 u8 data)
{
	...
	odm_set_rf_reg(dm, e_rf_path, RF_0x55, BIT(5), 1); /*power_trim based on 55[19:14]*/
	odm_set_rf_reg(dm, e_rf_path, RF_0xf5, BIT(18), 1); /*enable 55[14] for 0.5db step*/
	odm_set_rf_reg(dm, e_rf_path, RF_0xdf, BIT(7), 1); /*enter power_trim debug mode*/
	odm_set_rf_reg(dm, e_rf_path, RF_0xef, BIT(7), 1); /*write enable*/
	...
	odm_set_rf_reg(dm, e_rf_path, RF_0xdf, BIT(7), 0); /*leave power_trim debug mode*/
	odm_set_rf_reg(dm, e_rf_path, RF_0xef, BIT(7), 0); /*write disable*/

	RF_DBG(dm, DBG_RF_MP,
	       "[kfree] 8192F 0x55[19:14]=0x%X path=%d channel=%d\n",
	       odm_get_rf_reg(dm, e_rf_path, RF_0x55,
	       (BIT(19) | BIT(18) | BIT(17) | BIT(16) | BIT(15) | BIT(14))),
	       e_rf_path, channel_idx);
}
#if 0
/*
void
phydm_clear_kfree_to_rf_8192f(
	void		*dm_void,
	u8		e_rf_path,
	u8		data
)
...
void phydm_config_kfree(void *dm_void, u8 channel_to_sw)
{
	...
	if (dm->support_ic_type & ODM_RTL8814A)
		max_rf_path = 4; /*0~3*/
	else if (dm->support_ic_type & (ODM_RTL8812 | ODM_RTL8192E | ODM_RTL8822B | ODM_RTL8192F))
		max_rf_path = 2; /*0~1*/
		...
		if (dm->support_ic_type & ODM_RTL8710B)
			...
			for (i = 0; i < KFREE_BAND_NUM; i++) {
				for (j = 0; j < max_rf_path; j++)
					RF_DBG(dm, DBG_RF_MP, "[kfree] power_trim_data->bb_gain[%d][%d]=0x%X\n", i, j, power_trim_info->bb_gain[i][j]);
			}
		} else if (dm->support_ic_type & ODM_RTL8192F) {
			for (i = 0; i < KFREE_CH_NUM; i++) {
				for (j = 0; j < max_rf_path; j++)
		...
		if (*dm->band_type == ODM_BAND_2_4G && power_trim_info->flag & KFREE_FLAG_ON_2G) {
			if (!(dm->support_ic_type & ODM_RTL8192F)) {
				...
				for (rfpath = RF_PATH_A; rfpath < max_rf_path; rfpath++) {
					RF_DBG(dm, DBG_RF_MP, "[kfree] phydm_kfree(): channel_to_sw=%d PATH_%d bb_gain:0x%X\n", channel_to_sw, rfpath, power_trim_info->bb_gain[channel_idx][rfpath]);
				...
				if (channel_to_sw >= 10 && channel_to_sw <= 14)
					channel_idx = 2;
				for (rfpath = RF_PATH_A; rfpath < max_rf_path; rfpath++) {
					RF_DBG(dm, DBG_RF_MP, "[8192F kfree] phydm_kfree(): channel_to_sw=%d PATH_%d bb_gain:0x%X\n", channel_to_sw, rfpath, power_trim_info->bb_gain[channel_idx][rfpath]);
					phydm_set_kfree_to_rf_8192f(dm, rfpath, channel_idx, power_trim_info->bb_gain[channel_idx][rfpath]);
				}
			}
			...
			for (rfpath = RF_PATH_A; rfpath < max_rf_path; rfpath++) {
				RF_DBG(dm, DBG_RF_MP,
				       ...
				       channel_to_sw, rfpath,
				       power_trim_info->bb_gain[channel_idx][
				       rfpath]);
				phydm_set_kfree_to_rf(dm, rfpath, power_trim_info->bb_gain[channel_idx][rfpath]);
			}
		} else {
			...
			/*else if(dm->support_ic_type & ODM_RTL8192F){
				...
				if (channel_to_sw >= 9 && channel_to_sw <= 14)
					channel_idx = 2;
				for (rfpath = RF_PATH_A;  rfpath < max_rf_path; rfpath++)
					phydm_clear_kfree_to_rf_8192f(dm, rfpath, power_trim_info->bb_gain[channel_idx][rfpath]);
			}*/
#endif
		}
	}
