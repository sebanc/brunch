...
if [ -f ../chromiumos-stage3/chromiumos_stage3.tar.gz ]; then
	...
	curl -L https://github.com/sebanc/chromiumos-stage3/releases/download/r143-20251227/chromiumos_stage3_r143_20251227.tar.gz -o ./out/chromiumos_stage3.tar.gz || { echo "Failed to download the brunch toolchain"; exit 1; }
fi
...
if [ ! -z $1 ] && [ "$1" != "skip" ] ; then
	...
	cd rootfs
	curl -L https://dl.google.com/chromeos/reven/16463.51.0/stable-channel/chromeos_16463.51.0_reven_stable-channel_full_mp-v8.bin-gy4timjqmztdc26viexdtux3m4xeepye.signed -o ./update.signed || { echo "Failed to Download the OTA update"; exit 1; }
...
curl -L https://git.kernel.org/pub/scm/linux/kernel/git/wens/wireless-regdb.git/plain/regulatory.db -o ./out/regulatory.db || { echo "Failed to download the regulatory db"; exit 1; }
curl -L https://git.kernel.org/pub/scm/linux/kernel/git/wens/wireless-regdb.git/plain/regulatory.db.p7s -o ./out/regulatory.db.p7s || { echo "Failed to download the regulatory db"; exit 1; }
