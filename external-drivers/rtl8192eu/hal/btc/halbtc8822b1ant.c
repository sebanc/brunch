...
u32	glcoex_ver_date_8822b_1ant = 20170829;
u32	glcoex_ver_8822b_1ant = 0x4c;
u32	glcoex_ver_btdesired_8822b_1ant = 0x4b;
...
void halbtc8822b1ant_update_bt_link_info(IN struct btc_coexist *btcoexist)
{
	...
	if (bt_link_info->a2dp_exist) {
		...
		if (((coex_sta->bt_a2dp_vendor_id == 0) &&
			...
			coex_sta->bt_a2dp_vendor_id = val & 0xff;
		...
		if (((coex_sta->legacy_forbidden_slot == 0) &&
			...
			coex_sta->legacy_forbidden_slot = val & 0xffff;
			coex_sta->le_forbidden_slot = (val & 0xffff0000) >> 16;
		}
	}
		...
		*btcoexist)
{
	...
	if ((btcoexist->board_info.customerID == RT_CID_LENOVO_CHINA) &&
		...
		if (((bt_link_info->a2dp_only) && (coex_sta->is_bt_multi_link)) ||
			...
			/* for CPT_for_WiFi   */
			if (BTC_RSSI_LOW(wifi_rssi_state2) && (wifi_busy)) {
				halbtc8822b1ant_adjust_bt_tx_power(btcoexist, NORMAL_EXEC, 15);
