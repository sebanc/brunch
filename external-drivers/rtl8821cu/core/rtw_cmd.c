...
static u8 rtw_createbss_cmd(_adapter  *adapter, int flags, bool adhoc
	, u8 ifbmp, s16 req_ch, s8 req_bw, s8 req_offset)
{
...
inline u8 rtw_startbss_cmd(_adapter *adapter, int flags)
{
	return rtw_createbss_cmd(adapter, flags
		, 0
		, BIT(adapter->iface_id)
		, 0, REQ_BW_NONE, REQ_OFFSET_NONE /* excute entire AP setup cmd */
	);
}

inline u8 rtw_change_bss_chbw_cmd(_adapter *adapter, int flags, u8 ifbmp, s16 req_ch, s8 req_bw, s8 req_offset)
{
	return rtw_createbss_cmd(adapter, flags
		, 0
		, ifbmp
...
u8 rtw_dfs_master_hdl(_adapter *adapter)
{
	...
	rtw_change_bss_chbw_cmd(adapter, RTW_CMDF_DIRECTLY, ifbmp, REQ_CH_NONE, REQ_BW_ORI, REQ_OFFSET_NONE);
...
set_timer:
	_set_timer(&rfctl->dfs_master_timer, DFS_MASTER_TIMER_MS);
...
void rtw_dfs_master_timer_hdl(void *ctx)
{
	...
	rtw_dfs_master_cmd(dvobj_get_primary_adapter(dvobj), _TRUE);
}

static void rtw_dfs_master_enable(struct rf_ctl_t *rfctl, u8 ch, u8 bw, u8 offset)
{
	_adapter *adapter = dvobj_get_primary_adapter(rfctl_to_dvobj(rfctl));

	RTW_INFO("%s on %u,%u,%u\n", __func__, ch, bw, offset);

	if (rtw_is_cac_reset_needed(rfctl, ch, bw, offset) == _TRUE)
	...
	if (!rfctl->dfs_master_enabled) {
		RTW_INFO("%s set dfs_master_enabled\n", __func__);
...
static void rtw_dfs_master_disable(struct rf_ctl_t *rfctl, u8 ch, u8 bw, u8 offset, bool by_others)
{
	_adapter *adapter = dvobj_get_primary_adapter(rfctl_to_dvobj(rfctl));
	...
	if (rfctl->dfs_master_enabled) {
		...
		RTW_INFO("%s clear dfs_master_enabled\n", __func__);
		...
		rfctl->cac_start_time = rfctl->cac_end_time = RTW_CAC_STOPPED;
		_cancel_timer_ex(&rfctl->dfs_master_timer);
...
void rtw_dfs_master_status_apply(_adapter *adapter, u8 self_action)
{
	...
	if (self_action == MLME_OPCH_SWITCH) {
...
apply:
	...
	if (needed == _TRUE)
		rtw_dfs_master_enable(rfctl, u_ch, u_bw, u_offset);
	else
		rtw_dfs_master_disable(rfctl, u_ch, u_bw, u_offset, ld_sta_in_dfs);
}
