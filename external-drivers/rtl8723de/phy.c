...
static
void __rtw_phy_set_tx_power_sar(struct rtw_dev *rtwdev, u8 regd, u8 rfpath,
...
static
void _rtw_phy_set_tx_power_sar(struct rtw_dev *rtwdev, u8 regd, u8 rfpath,
			       u8 band, u8 chidx_start, u8 chidx_end, u8 sar_q3)
{
	...
	for (ch_idx = chidx_start; ch_idx <= chidx_end; ch_idx++)
		for (rs = 0; rs < RTW_RATE_SECTION_MAX; rs++)
			__rtw_phy_set_tx_power_sar(rtwdev, regd, rfpath, band,
						   rs, ch_idx, sar);
}
			      ...
			      u8 ch_start, u8 ch_end, u8 sar_q3)
{
	...
	if (band_start == band_end) {
		...
		if (chidx_start < 0 || chidx_end < 0)
			goto err;
		_rtw_phy_set_tx_power_sar(rtwdev, regd, rfpath, band_start,
	...
	if (chidx_start < 0)
		goto err;
	_rtw_phy_set_tx_power_sar(rtwdev, regd, rfpath, PHY_BAND_2G,
	...
	if (chidx_end < 0)
		goto err;
	_rtw_phy_set_tx_power_sar(rtwdev, regd, rfpath, PHY_BAND_5G,
				  0, chidx_end, sar_q3);
...
err:
	...
	*sar = (s8)rtwdev->chip->max_power_index;
