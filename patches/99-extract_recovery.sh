# This patch generates a script to produce "fake" recovery images from a running environment which can be used to build brunch

ret=0
cat >/roota/usr/sbin/extract-recovery <<INSTALL
#!/bin/bash
...
if [ \$(( (\$(df -k --output=avail "\${destination%/*}" | sed 1d) - ( \$image_size * 1024 * 1024)) )) -lt 0 ]; then echo "Not enough space to create image file"; exit 1; fi
...
chmod 0755 /roota/usr/sbin/extract-recovery
