# This patch increases the compatibility of the default cmt touchpad configuration
# An alternative touchpad configuration can be activated by using the option "alt_touchpad_config"
# A second alternative touchpad configuration can be activated by using the option "alt_touchpad_config2"

alt_touchpad_config=0
native_chromebook_image=0
for i in $(echo "$1" | sed 's#,# #g')
do
	if [ "$i" == "alt_touchpad_config" ]; then alt_touchpad_config=1; fi
	if [ "$i" == "alt_touchpad_config2" ]; then alt_touchpad_config=2; fi
	if [ "$i" == "alt_touchpad_config3" ]; then alt_touchpad_config=3; fi
...
if [ "$alt_touchpad_config" -eq 1 ]; then
	cat >/roota/etc/gesture/40-touchpad-cmt.conf <<TOUCHPADFIX
...
elif [ "$alt_touchpad_config" -eq 2 ]; then
	cat >/roota/etc/gesture/40-touchpad-cmt.conf <<TOUCHPADFIX
# Configure touchpads to use Chromium Multitouch (cmt) X input driver
Section "InputClass"
    Identifier      "touchpad"
...
elif [ "$alt_touchpad_config" -eq 3 ]; then
	cat >/roota/etc/gesture/40-touchpad-cmt.conf <<TOUCHPADFIX
# Configure touchpads to use Chromium Multitouch (cmt) X input driver
Section "InputClass"
    Identifier      "touchpad"
...
TOUCHPADFIX
	...
	cat >/roota/etc/gesture/40-touchpad-cmt.conf <<TOUCHPAD
# Configure touchpads to use Chromium Multitouch (cmt) X input driver
Section "InputClass"
    Identifier      "touchpad"
    MatchIsTouchpad "on"
