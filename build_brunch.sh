...
if [ -f ../chromiumos-stage3/chromiumos_stage3.tar.gz ]; then
	...
	curl -L https://github.com/sebanc/chromiumos-stage3/releases/download/r149-20260704/chromiumos_stage3_r149_20260704.tar.gz -o ./out/chromiumos_stage3.tar.gz || { echo "Failed to download the brunch toolchain"; exit 1; }
fi
...
if [ ! -z $1 ] && [ "$1" != "skip" ] ; then
	...
	cd rootfs
	curl -L https://dl.google.com/chromeos/reven/16667.55.0/stable-channel/chromeos_16667.55.0_reven_stable-channel_full_RevenMPKeys-v10.bin-gzqtgy3ehftdb4uetk24ajqxvoelrzx5.signed -o ./update.signed || { echo "Failed to Download the OTA update"; exit 1; }
...
ln -s kernel ./chroot/home/chronos/rootc/kernel-chromebook-4.19 || { echo "Failed to make the legacy kernel symlink"; exit 1; }
ln -s kernel ./chroot/home/chronos/rootc/kernel-chromebook-5.4 || { echo "Failed to make the legacy kernel symlink"; exit 1; }
