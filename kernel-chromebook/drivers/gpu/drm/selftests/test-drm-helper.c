...
static void set_clip(struct drm_rect *plane_clip,
		     int crtc_x, int crtc_y,
		     unsigned crtc_w, unsigned crtc_h)
{
	plane_clip->x1 = crtc_x;
	plane_clip->y1 = crtc_y;
	plane_clip->x2 = crtc_w;
	plane_clip->y2 = crtc_h;
}
...
static int igt_check_plane_state(void *ignored)
{
	...
	set_clip(&plane_clip, 0, 0, fb.width, fb.height);
	...
	plane_state.rotation = DRM_ROTATE_90 | DRM_REFLECT_X;
	ret = drm_plane_helper_check_state(&plane_state, &plane_clip,
					   DRM_PLANE_HELPER_NO_SCALING,
					   DRM_PLANE_HELPER_NO_SCALING,
					   false, false);
	...
	set_clip(&plane_clip, 0, 0, 1023, 767);
	ret = drm_plane_helper_check_state(&plane_state, &plane_clip,
					   DRM_PLANE_HELPER_NO_SCALING,
					   DRM_PLANE_HELPER_NO_SCALING,
					   false, false);
	...
	set_clip(&plane_clip, 0, 0, 1024, 768);
	ret = drm_plane_helper_check_state(&plane_state, &plane_clip,
					   0x8001,
					   DRM_PLANE_HELPER_NO_SCALING,
					   false, false);
	...
	set_clip(&plane_clip, 1022, 766, 4, 4);
	ret = drm_plane_helper_check_state(&plane_state, &plane_clip,
					   DRM_PLANE_HELPER_NO_SCALING,
					   0x10001,
					   true, false);
	...
	set_clip(&plane_clip, -2, -2, 1028, 772);
	ret = drm_plane_helper_check_state(&plane_state, &plane_clip,
					   DRM_PLANE_HELPER_NO_SCALING,
					   0x10001,
					   false, false);
