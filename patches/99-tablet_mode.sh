...
ACTION=="add", SUBSYSTEMS=="usb", ATTRS{idVendor}=="045E", ATTRS{idProduct}=="09C0", RUN+="/bin/bash -c 'echo 0 > /sys/bus/platform/devices/tablet_mode_switch.0/tablet_mode'"
ACTION=="remove", SUBSYSTEMS=="usb", ATTRS{idVendor}=="045E", ATTRS{idProduct}=="09C0", RUN+="/bin/bash -c 'echo 1 > /sys/bus/platform/devices/tablet_mode_switch.0/tablet_mode'"
UDEVRULE
