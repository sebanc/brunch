...
if [ ! -f "./kernels/kernel-$kernel/out/arch/x86/boot/bzImage" ]; then echo "The kernel $kernel has to be built first"; exit 1; fi
...
cd rootfs
curl -L https://dl.google.com/chromeos/rammus/14092.46.0/stable-channel/chromeos_14092.46.0_rammus_stable-channel_full_mp-v2.bin-gyytezjqme4tjjz4yvqodfvcgwpetbiu.signed -o ./update.signed
...
cp -r ./kernels/kernel-"$kernel" ./chroot/tmp/kernel
...
git clone --depth=1 -b master git://git.kernel.org/pub/scm/linux/kernel/git/firmware/linux-firmware.git firmwares
cd ./firmwares
rm -rf ./.git
rm -rf ./bnx2x
rm -rf ./dpaa2
...
cp -r ../../../../extra-firmwares/* ./
mkdir -p ../rootc/lib/firmware
mv amd-ucode.img intel-ucode.img ../rootc/lib/firmware/
