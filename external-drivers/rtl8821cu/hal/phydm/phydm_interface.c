...
void odm_set_timer(struct dm_struct *dm, struct phydm_timer_list *timer, u32 ms_delay)
{
#if (DM_ODM_SUPPORT_TYPE & ODM_AP)
...
void odm_release_timer(struct dm_struct *dm, struct phydm_timer_list *timer)
{
	...
	if (timer == 0) {
		PHYDM_DBG(dm, ODM_COMP_INIT,
			  "=====>%s, The timer is NULL! Please check it!\n",
			  __func__);
		return;
	}
...
#if (DM_ODM_SUPPORT_TYPE & ODM_WIN)
	void *adapter = dm->adapter;
	PHY_SetTxPowerIndexByRateSection(adapter, path, channel, rate_section);
#elif (DM_ODM_SUPPORT_TYPE == ODM_CE) && defined(DM_ODM_CE_MAC80211)
	void *adapter = dm->adapter;

	phy_set_tx_power_index_by_rs(adapter, channel, path, rate_section);
#elif (DM_ODM_SUPPORT_TYPE & ODM_CE)
			  ...
			  u8 band_width, u8 channel)
{
	...
	return PHY_GetTxPowerIndex(dm->adapter, path, tx_rate, (CHANNEL_WIDTH)band_width, channel);
#elif (DM_ODM_SUPPORT_TYPE & ODM_CE) && defined(DM_ODM_CE_MAC80211)
	void *adapter = dm->adapter;
	...
	return PHY_GetTxPowerIndex(dm->adapter, path, tx_rate, band_width, channel);
#endif
}
