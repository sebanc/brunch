...
if [[ $device = 1 ]]; then
	...
	if [ -e /dev/loop-control ]; then
		...
		if [ $(( ($(df -k --output=avail "$path" | sed 1d) - ( $image_size * 1024 * 1024)) )) -lt 0 ]; then echo "Not enough space to create image file"; exit 1; fi
		...
		for (( i=1; i<=12; i++ )); do
			...
			case $i in
				2)
				dd if="$loopdevice"p4 ibs=1M status=none | pv | dd of="$destloop"p2 obs=1M status=none
				;;
				5)
				dd if="$loopdevice"p3 ibs=1M status=none | pv | dd of="$destloop"p5 obs=1M status=none
				;;
				7)
				dd if="$(dirname $0)/rootc.img" ibs=1M status=none | pv | dd of="$destloop"p7 obs=1M status=none
				;;
				12)
				if [[ $legacy_boot = 1 ]]; then
					dd if="$(dirname $0)/efi_legacy.img" ibs=1M status=none | pv | dd of="$destloop"p12 obs=1M status=none
				else
					dd if="$(dirname $0)/efi_secure.img" ibs=1M status=none | pv | dd of="$destloop"p12 obs=1M status=none
				fi
				;;
				*)
				dd if="$loopdevice"p"$i" ibs=1M status=none | pv | dd of="$destloop"p"$i" obs=1M status=none
				;;
			esac
		done
		losetup -d "$loopdevice"
