...
if [ "$type" == "Dualboot (create an image)" ]; then
	...
	if [ -z "$wsl" ] && ([ "$(grep -o '^ID=[^,]\+' /etc/os-release | cut -d'=' -f2)" == "ubuntu" ] || [ "$(grep -o '^ID=[^,]\+' /etc/os-release | cut -d'=' -f2)" == "linuxmint" ] || [ "$(grep -o '^ID=[^,]\+' /etc/os-release | cut -d'=' -f2)" == "Zorin OS" ]); then remove_tpm="\n	rmmod tpm"; fi
...
menuentry \"ChromeOS (settings)\" --class \"brunch-settings\" {$remove_tpm
	search --no-floppy --set=root --file "$img_path"
	loopback loop "$img_path"
	source (loop,12)/efi/boot/settings.cfg
