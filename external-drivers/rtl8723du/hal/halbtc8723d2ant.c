...
static u8 halbtc8723d2ant_wifi_rssi_state(struct btc_coexist *btcoexist,
	   u8 *pprewifi_rssi_state, u8 level_num, u8 rssi_thresh,
				   u8 rssi_thresh1)
{
	int			wifi_rssi = 0;
...
static void halbtc8723d2ant_update_wifi_channel_info(struct btc_coexist *btcoexist,
		u8 type)
{
...
static void halbtc8723d2ant_set_fw_low_penalty_ra(struct btc_coexist
...
static bool halbtc8723d2ant_is_wifibt_status_changed(struct btc_coexist
		*btcoexist)
{
...
static void halbtc8723d2ant_monitor_bt_enable_disable(struct btc_coexist *btcoexist)
{
...
static u32 halbtc8723d2ant_ltecoex_indirect_read_reg(struct btc_coexist *btcoexist,
		u16 reg_addr)
{
	u32 delay_count = 0;
...
static void halbtc8723d2ant_ltecoex_indirect_write_reg(struct btc_coexist
		*btcoexist,
		u16 reg_addr, u32 bit_mask, u32 reg_value)
{
	u32 val, i, bitpos = 0, delay_count = 0;
...
static void halbtc8723d2ant_ltecoex_pathcontrol_owner(struct btc_coexist *btcoexist,
		bool wifi_control)
{
	u8 val;
...
static void halbtc8723d2ant_ltecoex_set_coex_table(struct btc_coexist *btcoexist,
...
static void halbtc8723d2ant_set_fw_ignore_wlan_act(struct btc_coexist *btcoexist,
		bool enable)
{
	u8			h2c_parameter[1] = {0};
...
/* HID+A2DP+PAN(EDR) */
static void halbtc8723d2ant_action_hid_a2dp_pan_edr(struct btc_coexist *btcoexist)
{
	static u8	prewifi_rssi_state = BTC_RSSI_STATE_LOW;
	static u8	pre_bt_rssi_state = BTC_RSSI_STATE_LOW;
	u8	wifi_rssi_state, bt_rssi_state;
...
static void halbtc8723d2ant_action_wifi_native_lps(struct btc_coexist *btcoexist)
{
	halbtc8723d2ant_coex_table_with_type(btcoexist, NORMAL_EXEC, 2);
	halbtc8723d2ant_ps_tdma(btcoexist, NORMAL_EXEC, false, 0);
}

static void halbtc8723d2ant_action_wifi_multi_port(struct btc_coexist *btcoexist)
{
	halbtc8723d2ant_adjust_wl_tx_power(btcoexist, NORMAL_EXEC, 0xb2);
	halbtc8723d2ant_adjust_bt_tx_power(btcoexist, NORMAL_EXEC, 15);
	halbtc8723d2ant_adjust_wl_rx_gain(btcoexist, NORMAL_EXEC, false);
...
static void halbtc8723d2ant_action_wifi_linkscan_process(struct btc_coexist *btcoexist)
{
	struct	btc_bt_link_info *bt_link_info = &btcoexist->bt_link_info;
...
static void halbtc8723d2ant_action_wifi_not_connected(struct btc_coexist *btcoexist)
{
	halbtc8723d2ant_adjust_wl_tx_power(btcoexist, NORMAL_EXEC, 0xb2);
	halbtc8723d2ant_adjust_bt_tx_power(btcoexist, NORMAL_EXEC, 0);
	halbtc8723d2ant_adjust_wl_rx_gain(btcoexist, NORMAL_EXEC, false);
...
static u32 halbtc8723d2ant_psd_log2base(struct btc_coexist *btcoexist, u32 val)
{
	u8	j;
...
static void halbtc8723d2ant_psd_show_antenna_detect_result(struct btc_coexist
		*btcoexist)
{
...
static void halbtc8723d2ant_psd_showdata(struct btc_coexist *btcoexist)
{
	u8		*cli_buf = btcoexist->cli_buf;
...
static void halbtc8723d2ant_psd_maxholddata(struct btc_coexist *btcoexist,
				     u32 gen_count)
{
	u32 i = 0;
...
static u32 halbtc8723d2ant_psd_getdata(struct btc_coexist *btcoexist, u32 point)
{
...
static bool halbtc8723d2ant_psd_sweep_point(struct btc_coexist *btcoexist,
...
static bool halbtc8723d2ant_psd_antenna_detection(struct btc_coexist
		*btcoexist)
{
	u32	i = 0;
	...
	do {
		...
		case 0:
			...
			BTC_TRACE(trace_buf);
			BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
				"xxxxxxx AntennaDetect(), cent_freq=%d,  offset = %d, span = %d, sweep_count = %d\n",
				    wlpsd_cent_freq,
				    wlpsd_offset,
				    wlpsd_span,
		...
		case 2:	/* Pre-sweep background psd */
			...
			if (psd_scan->ant_det_pre_psdscan_peak_val >
				...
				BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
					"xxxxxxx AntennaDetect(), Abort Antenna Detection!! because background = %d > thres (%d)\n",
					psd_scan->ant_det_pre_psdscan_peak_val /
					    100,
					BT_8723D_2ANT_ANTDET_PSDTHRES_BACKGROUND
					    + psd_scan->ant_det_thres_offset);
				...
				BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
					"xxxxxxx AntennaDetect(), Start Antenna Detection!! because background = %d <= thres (%d)\n",
					psd_scan->ant_det_pre_psdscan_peak_val /
					    100,
					BT_8723D_2ANT_ANTDET_PSDTHRES_BACKGROUND
					    + psd_scan->ant_det_thres_offset);
...
static bool halbtc8723d2ant_psd_antenna_detection_check(struct btc_coexist
		*btcoexist)
{
...
void ex_halbtc8723d2ant_ips_notify(struct btc_coexist *btcoexist, u8 type)
{
...
void ex_halbtc8723d2ant_lps_notify(struct btc_coexist *btcoexist, u8 type)
{
