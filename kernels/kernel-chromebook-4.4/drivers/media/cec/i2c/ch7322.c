...
static void ch7322_cec_notify(struct cec_adapter *adap, u16 pa)
{
...
static int ch7322_probe(struct i2c_client *client)
{
	...
	if (hdmi_dev) {
		notifier = cec_notifier_get_conn(hdmi_dev, port_name);
		if (!notifier) {
			ret = -ENOMEM;
			goto err_cec;
		}
		...
		cec_notifier_register(notifier, ch7322->cec, ch7322_cec_notify);
	}
...
err_notifier:
	if (notifier)
		cec_notifier_unregister(notifier);
err_cec:
