...
image="$source"
if [ ! -z "$zenity" ]; then
	write_base_table "$destination" > /dev/null 2>&1 >(zenity --height=480 --width=640 --title="Brunch installer" --progress --auto-close --pulsate --text="Creating partition table..." --percentage=100)
else
	echo "Creating partition table..."
	write_base_table "$destination" > /dev/null 2>&1
fi
for (( i=1; i<=12; i++ )); do
	case $i in
		2)
	...
	if [ ! -z "$zenity" ]; then
		dd if="$image" ibs=512 count="$size" skip="$source_start" status=none | pv -n -s $(( $size * 512 )) 2> >(zenity --height=480 --width=640 --title="Brunch installer" --progress --auto-close --text="Writing partition $i..." --percentage=0 --no-cancel) | dd of="$destination" obs=512 seek="$destination_start" conv=notrunc status=none || error $i
	else
		dd if="$image" ibs=512 count="$size" skip="$source_start" status=none | pv -s $(( $size * 512 )) | dd of="$destination" obs=512 seek="$destination_start" conv=notrunc status=none || error $i
	fi
done
}
...
local size
dd if=/dev/zero of="$destination" bs=1G seek=$image_size count=0 status=none
if [ ! -z "$zenity" ]; then
	write_base_table "$destination" > /dev/null 2>&1 >(zenity --height=480 --width=640 --title="Brunch installer" --progress --auto-close --pulsate --text="Creating partition table..." --percentage=100)
else
	echo "Creating partition table..."
...
destination_loop=$(losetup --show -fP "$destination")
for (( i=1; i<=12; i++ )); do
	case $i in
		2)
	...
	if [ ! -z "$zenity" ]; then
		dd if="$source_part" ibs=1M status=none | pv -n -s $(( $size * 512 )) 2> >(zenity --height=480 --width=640 --title="Brunch installer" --progress --auto-close --text="Writing partition $i..." --percentage=0 --no-cancel) | dd of="$destination_loop"p"$i" obs=1M status=none || error $i
	else
		dd if="$source_part" ibs=1M status=none | pv -s $(( $size * 512 )) | dd of="$destination_loop"p"$i" obs=1M status=none || error $i
	fi
done
...
if [ "$type" == "Dualboot (create an image)" ]; then
	if [ ! -z "$wsl" ]; then
		if [ ! -z "$zenity" ]; then
			img_uuid=$(zenity  --height=480 --width=640 --title="Brunch installer" --entry --text "The ChromeOS disk image has been created.\n\n In order to prepare the dual boot grub configuration, right click on the start menu icon, select Windows Terminal, run the command in the below textbox and replace the text with the command output.\n(only the figures and dashes like: XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX)" --entry-text "mountvol $(echo ${destination:5:1} | tr a-z A-Z):\\ /L")
		else
			read -p "The ChromeOS disk image has been created."$'\n'$'\n'"In order to prepare the dual boot grub configuration, right click on the start menu icon, select Windows Terminal, run the command \"mountvol $(echo ${destination:5:1} | tr a-z A-Z):\\ /L\" and copy the output here (only the figures and dashes like: XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX):"$'\n' img_uuid
			echo -e ""
		fi
	else
	...
	config="menuentry \"ChromeOS\" --class \"brunch\" {
...
menuentry \"ChromeOS (settings)\" --class \"brunch-settings\" {
	...
	echo "$config" > "$destination".grub.txt
	if [ ! -z "$wsl" ]; then
		grubinstall="The ChromeOS dual boot disk image has been created and the config needed to boot ChromeOS from Grub2Win has been generated in the file:\n$(echo ${destination:5:1} | tr a-z A-Z):\\\\$(echo ${destination:7} | sed 's@\/@\\\\@g').grub.txt\n\nNow, install Grub2Win and launch it, click on \"Manage Boot Menu\" -> \"Add a new entry\" -> set \"Type\" as \"Create user section\", open the file $(echo ${destination:5:1} | tr a-z A-Z):\\\\$(echo ${destination:7} | sed 's@\/@\\\\@g').grub.txt and copy its content in the Grub2Win notepad window, save and close the Grub2Win notepad window then click \"Apply\" and \"OK\"."
...
if [ ! -z "$zenity" ]; then
	...
	rm -rf "$destination"
	if [[ ! $destination == *"/"* ]]; then path="."; else path="$(realpath "$destination")"; path="$(echo ${path} | sed 's@[^/]*$@@')"; fi
	...
	if [ $(( ($(df -k --output=avail "$path" | sed 1d) / 1024 / 1024) - 14 )) -lt 0 ]; then  zenity --height=480 --width=640 --error --text="Not enough space to create image file, the minimum size is 14 GB. Verify that the path you have selected points to a partition with more than 14GB available."; dualboot; fi
	if [ -z "$image_size" ]; then image_size=$(zenity --height=480 --width=640 --title="Brunch installer" --scale --text "This partition has $(( ($(df -k --output=avail $path | sed 1d) / 1024 / 1024) )) GB available.\n how much would you like to allocate for ChromeOS ?\n (Around 10GB will be occupied by system files, the remaining space will be available).\n" --min-value=14 --max-value=$(( ($(df -k --output=avail $path | sed 1d) / 1024 / 1024) )) --value=14 --step 1); fi
...
if [ $# -eq 0 ]; then
	...
	if [ ! $(dd if="$source" bs=1 count=4 status=none | od -A n -t x1 | sed 's/ //g') == '33c0fa8e' ] || [ $(cgpt show -i 12 -b "$source") -eq 0 ] || [ $(cgpt show -i 13 -b "$source") -gt 0 ] || [ ! $(cgpt show -i 3 -l "$source") == 'ROOT-A' ]; then echo "$source is not a valid ChromeOS recovey image (have you unzipped it ?)"; exit 1; fi
	...
	while [ $# -gt 0 ]; do
		case "$1" in
			...
			if [ ! $(dd if="$1" bs=1 count=4 status=none | od -A n -t x1 | sed 's/ //g') == '33c0fa8e' ] || [ $(cgpt show -i 12 -b "$1") -eq 0 ] || [ $(cgpt show -i 13 -b "$1") -gt 0 ] || [ ! $(cgpt show -i 3 -l "$1") == 'ROOT-A' ]; then echo "$1 is not a valid ChromeOS recovey image (have you unzipped it ?)"; exit 1; fi
			source="$1"
			;;
			-dst | --destination)
			shift
