...
{
	int last_buffer_completed, last_buffer_submitted;

	if (ipts->state != IPTS_STA_RAW_DATA_STARTED)
		return;

	last_buffer_completed = ipts->resources.last_buffer_completed;
	last_buffer_submitted = *ipts->resources.last_buffer_submitted;

	if (last_buffer_completed == last_buffer_submitted)
		return;

	ipts->resources.last_buffer_completed = last_buffer_submitted;
	handle_output_buffers(ipts, last_buffer_completed, last_buffer_submitted);
}
