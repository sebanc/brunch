...
#define CL_OP_TIMEOUT_MS		300
#define CL_IOC_TIMEOUT_MS		300
#define CL_STREAM_INDEX			0
...
void hda_cldma_fill(struct hda_cldma *cl)
{
	...
	if (offset + size > cl->buffer_size) {
		...
		memcpy(cl->dmab_data.area + offset, cl->pos, ss);
		offset = 0;
		size -= ss;
		cl->pos += ss;
		cl->remaining -= ss;
	...
	memcpy(cl->dmab_data.area + offset, cl->pos, size);
	cl->pos += size;
	cl->remaining -= size;
...
static void cldma_memcpy_work(struct work_struct *work)
{
	...
	while (true) {
		ret = wait_for_completion_timeout(&cl->completion,
				msecs_to_jiffies(CL_IOC_TIMEOUT_MS));
		if (!ret) {
		...
		snd_hdac_adsp_updatel(cl, AZX_ADSP_REG_ADSPIC,
				      AZX_ADSP_ADSPIC_CLDMA,
				      AZX_ADSP_ADSPIC_CLDMA);
	}
}
...
int hda_cldma_start(struct hda_cldma *cl)
{
	...
	/* enable interrupts */
	snd_hdac_adsp_updatel(cl, AZX_ADSP_REG_ADSPIC,
			      AZX_ADSP_ADSPIC_CLDMA, AZX_ADSP_ADSPIC_CLDMA);
...
static irqreturn_t cldma_irq_handler(int irq, void *dev_id)
{
	...
	adspis = snd_hdac_adsp_readl(cl, AZX_ADSP_REG_ADSPIS);
	if (adspis == UINT_MAX)
		return IRQ_NONE;
	if (!(adspis & AZX_ADSP_ADSPIS_CLDMA))
		return IRQ_NONE;
