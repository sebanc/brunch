...
void rtl8812au_interface_configure(_adapter *padapter)
{
	...
	pHalData->rxagg_mode = RX_AGG_USB; /* todo: change to USB_RX_AGG_DMA for 8821A*/
...
#define HAL_INIT_PROFILE_TAG(stage) do {} while (0)
	...
	*/
	...
	if (IS_HARDWARE_TYPE_8812AU(Adapter)) {
		if (GET_HAL_RFPATH(Adapter) == RF_1T1R)
			PHY_BB8812_Config_1T(Adapter);
		else if (GET_HAL_RFPATH(Adapter) == RF_1T2R)
...
void rtl8812au_set_hal_ops(_adapter *padapter)
{
	...
	pHalFunc->hal_mgmt_xmitframe_enqueue = &rtl8812au_hal_mgmt_xmitframe_enqueue;
#endif
