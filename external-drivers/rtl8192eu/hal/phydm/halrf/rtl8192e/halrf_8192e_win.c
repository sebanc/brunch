...
)
{
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD, ("pwr_tracking_limit_cck=%d      pwr_tracking_limit_ofdm=%d\n", pwr_tracking_limit_cck, pwr_tracking_limit_ofdm));
	...
	if (method == TXAGC) {
		...
		if (rf_path == RF_PATH_A) {
			PHY_SetTxPowerIndexByRateSection(adapter, RF_PATH_A, *p_dm->p_channel, CCK);
			PHY_SetTxPowerIndexByRateSection(adapter, RF_PATH_A, *p_dm->p_channel, OFDM);
			PHY_SetTxPowerIndexByRateSection(adapter, RF_PATH_A, *p_dm->p_channel, HT_MCS0_MCS7);
			PHY_SetTxPowerIndexByRateSection(adapter, RF_PATH_A, *p_dm->p_channel, HT_MCS8_MCS15);
		} else {
			PHY_SetTxPowerIndexByRateSection(adapter, RF_PATH_B, *p_dm->p_channel, CCK);
			PHY_SetTxPowerIndexByRateSection(adapter, RF_PATH_B, *p_dm->p_channel, OFDM);
			PHY_SetTxPowerIndexByRateSection(adapter, RF_PATH_B, *p_dm->p_channel, HT_MCS0_MCS7);
			PHY_SetTxPowerIndexByRateSection(adapter, RF_PATH_B, *p_dm->p_channel, HT_MCS8_MCS15);
		}
#else
...
)
{
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD, ("Power Tracking tx_rate=0x%X\n", tx_rate));
	ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD, ("Call get_delta_swing_table_8192e Power Tracking tx_rate=0x%X\n", tx_rate));
...
)
{
	...
	*/
	...
	odm_set_bb_reg(p_dm, 0x040, 0x20, 0x0);
...
)
{
	...
	if (t == 0) {
		...
		_phy_save_adda_registers_92e(p_dm, ADDA_REG, p_rf_calibrate_info->ADDA_backup, IQK_ADDA_REG_NUM);
		_phy_save_mac_registers_92e(p_dm, IQK_MAC_REG, p_rf_calibrate_info->IQK_MAC_backup);
		_phy_save_adda_registers_92e(p_dm, IQK_BB_REG_92C, p_rf_calibrate_info->IQK_BB_backup, IQK_BB_REG_NUM);
	}
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("IQ Calibration for %s for %d times\n", (is2T ? "2T2R" : "1T1R"), t));
...
#endif
	/* MAC settings */
	_phy_mac_setting_calibration_92e(p_dm, IQK_MAC_REG, p_rf_calibrate_info->IQK_MAC_backup);
	...
	* 	ODM_RT_TRACE(p_dm,ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("IQK setting!\n")); */
	...
	if (t != 0) {
		...
		_phy_reload_adda_registers_92e(p_dm, ADDA_REG, p_rf_calibrate_info->ADDA_backup, IQK_ADDA_REG_NUM);
		/* Reload MAC parameters*/
		_phy_reload_mac_registers_92e(p_dm, IQK_MAC_REG, p_rf_calibrate_info->IQK_MAC_backup);
		_phy_reload_adda_registers_92e(p_dm, IQK_BB_REG_92C, p_rf_calibrate_info->IQK_BB_backup, IQK_BB_REG_NUM);
...
)
{
 ...
 /*for ODM_WIN*/
	if (is_recovery && (!p_dm->is_in_hct_test)) {/*YJ,add for PowerTest,120405*/
		...
		_phy_reload_adda_registers_92e(p_dm, IQK_BB_REG_92C, p_rf_calibrate_info->IQK_BB_backup_recover, 9);
		return;
	}
	...
	_phy_save_adda_registers_92e(p_dm, IQK_BB_REG_92C, p_rf_calibrate_info->IQK_BB_backup_recover, IQK_BB_REG_NUM);
