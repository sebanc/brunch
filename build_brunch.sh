...
if [ ! -z $1 ] && [ "$1" != "skip" ] ; then
	...
	cd rootfs
	curl -L https://dl.google.com/chromeos/rammus/15572.57.0/stable-channel/chromeos_15572.57.0_rammus_stable-channel_full_mp-v3.bin-gy2tcnjrga2wgqrbgsm364wfbobbrkeb.signed -o ./update.signed || { echo "Failed to Download the OTA update"; exit 1; }
...
ln -s kernel-5.15 ./chroot/home/chronos/rootc/kernel-macbook || { echo "Failed to make the macbook kernel symlink"; exit 1; }
ln -s kernel-5.15 ./chroot/home/chronos/rootc/kernel-macbook-t2 || { echo "Failed to make the macbook kernel symlink"; exit 1; }
