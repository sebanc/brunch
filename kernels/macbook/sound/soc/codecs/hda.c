	...
	for (i = 0; i < pcm_count; i++, pcm = list_next_entry(pcm, list)) {
		...
		stream->stream_name =
			devm_kasprintf(dev, GFP_KERNEL, "%s %s",
				       pcm->name, snd_pcm_direction_name(dir));
				   ...
				   struct snd_soc_component *component)
{
	...
	list_for_each_entry(pcm, &codec->pcm_list_head, list) {
		...
		snd_soc_dai_init_dma_data(dai, &pcm->stream[0],
					  &pcm->stream[1]);
		drvs++;
	}
...
static int hda_hdev_attach(struct hdac_device *hdev)
{
	...
	return snd_soc_register_component(&hdev->dev, comp_drv,
					  &card_binder_dai, 1);
}
...
MODULE_DESCRIPTION("Generic Intel HD-Audio codec driver");
