...
#if (DM_ODM_SUPPORT_TYPE & ODM_AP)
	phy_iq_calibrate_8192e(p_dm, false);
#else
...
)
{
	if (method == TXAGC) {
		...
		pwr = phy_query_bb_reg(adapter, REG_TX_AGC_A_RATE18_06, 0xFF);
		pwr += (p_dm->bb_swing_idx_ofdm[RF_PATH_A] - p_dm->bb_swing_idx_ofdm_base[RF_PATH_A]);
		...
		if (!p_dm->rf_calibrate_info.is_cck_in_ch14) {
			odm_write_1byte(p_dm, 0xa22, cck_swing_table_ch1_ch13_92e[p_dm->bb_swing_idx_cck][0]);
			odm_write_1byte(p_dm, 0xa23, cck_swing_table_ch1_ch13_92e[p_dm->bb_swing_idx_cck][1]);
			odm_write_1byte(p_dm, 0xa24, cck_swing_table_ch1_ch13_92e[p_dm->bb_swing_idx_cck][2]);
			odm_write_1byte(p_dm, 0xa25, cck_swing_table_ch1_ch13_92e[p_dm->bb_swing_idx_cck][3]);
			odm_write_1byte(p_dm, 0xa26, cck_swing_table_ch1_ch13_92e[p_dm->bb_swing_idx_cck][4]);
			odm_write_1byte(p_dm, 0xa27, cck_swing_table_ch1_ch13_92e[p_dm->bb_swing_idx_cck][5]);
			odm_write_1byte(p_dm, 0xa28, cck_swing_table_ch1_ch13_92e[p_dm->bb_swing_idx_cck][6]);
			odm_write_1byte(p_dm, 0xa29, cck_swing_table_ch1_ch13_92e[p_dm->bb_swing_idx_cck][7]);
		} else {
			odm_write_1byte(p_dm, 0xa22, cck_swing_table_ch14_92e[p_dm->bb_swing_idx_cck][0]);
			odm_write_1byte(p_dm, 0xa23, cck_swing_table_ch14_92e[p_dm->bb_swing_idx_cck][1]);
			odm_write_1byte(p_dm, 0xa24, cck_swing_table_ch14_92e[p_dm->bb_swing_idx_cck][2]);
			odm_write_1byte(p_dm, 0xa25, cck_swing_table_ch14_92e[p_dm->bb_swing_idx_cck][3]);
			odm_write_1byte(p_dm, 0xa26, cck_swing_table_ch14_92e[p_dm->bb_swing_idx_cck][4]);
			odm_write_1byte(p_dm, 0xa27, cck_swing_table_ch14_92e[p_dm->bb_swing_idx_cck][5]);
			odm_write_1byte(p_dm, 0xa28, cck_swing_table_ch14_92e[p_dm->bb_swing_idx_cck][6]);
			odm_write_1byte(p_dm, 0xa29, cck_swing_table_ch14_92e[p_dm->bb_swing_idx_cck][7]);
		}

		/* Adjust BB swing by OFDM IQ matrix */
		if (rf_path == RF_PATH_A) {
			set_iqk_matrix_8192e(p_dm, p_dm->bb_swing_idx_ofdm[RF_PATH_A], RF_PATH_A,
			...
			set_iqk_matrix_8192e(p_dm, p_dm->bb_swing_idx_ofdm[RF_PATH_B], RF_PATH_B,
...
)
{
	...
	* 	ODM_RT_TRACE(p_dm,ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("path-A Rx IQK modify RXIQK mode table!\n")); */
	...
	odm_set_rf_reg(p_dm, RF_PATH_A, 0x56, RFREGOFFSETMASK, 0x51100);
...
)
{
	...
	*/
#if 1
	odm_set_bb_reg(p_dm, 0x520, MASKBYTE2, 0xff);
#else
	...
	while ((((odm_get_rf_reg(p_dm, 0x0, 0x0, bMaskDWord) & 0xf0000)  >> 16) == 0x2) && (((odm_get_rf_reg(p_dm, 0x1, 0x0, bMaskDWord) & 0xf0000)  >> 16) == 0x2))
		ODM_delay_ms(1);
...
)
{
	...
	odm_set_rf_reg(p_dm, 0x0, 0x0, RFREGOFFSETMASK, 0x10000);
	odm_set_bb_reg(p_dm, REG_FPGA0_IQK, 0xffffff00, 0x808000);
}
...
)
{
	...
	/*
	 *	odm_set_bb_reg(p_dm, REG_FPGA0_XAB_RF_INTERFACE_SW, BIT(10), 0x01);
	...
	/*	if(is2T)
	 *	{
	 *		odm_set_bb_reg(p_dm, REG_FPGA0_XA_LSSI_PARAMETER, MASKDWORD, 0x00010000);
	 *		odm_set_bb_reg(p_dm, REG_FPGA0_XB_LSSI_PARAMETER, MASKDWORD, 0x00010000);
	 *	} */
	...
	if (is2T) {
		...
		for (i = 0 ; i < retry_count ; i++) {
			...
			if (path_bok == 0x01) {
				...
				ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("path B Tx IQK Fail!!\n"));
				result[t][4] = 0x100;
				result[t][5] = 0x0;
			}
#if 0
...
)
{
	...
	_phy_lc_calibrate_8192e(p_dm, false);
}
...
)
{
	...
	if (p_adapter->hw_init_completed == false)
#endif
	{
		u8	u1b_tmp;
		u1b_tmp = odm_read_1byte(p_dm, REG_LEDCFG2) | BIT(7);
		odm_write_1byte(p_dm, REG_LEDCFG2, u1b_tmp);
		/* odm_set_bb_reg(p_dm, REG_LEDCFG0, BIT23, 0x01); */
		odm_set_bb_reg(p_dm, REG_FPGA0_XAB_RF_PARAMETER, BIT(13), 0x01);
	}
#endif
	...
	if (is2T) {	/* 92C */
		...
		odm_set_bb_reg(p_dm, REG_FPGA0_XAB_RF_INTERFACE_SW, BIT(8) | BIT(9), 0x0);
		odm_set_bb_reg(p_dm, 0x914, MASKLWORD, 0x0201);		  			  /* Set up the ant mapping table */

		if (is_main) {
			/* odm_set_bb_reg(p_dm, REG_FPGA0_XA_RF_INTERFACE_OE, BIT(8)|BIT9, 0x2);		  */ /* Tx Main (SW control)(The right antenna) */
			/* 4 [ Tx ] */
			odm_set_bb_reg(p_dm, REG_FPGA0_XA_RF_INTERFACE_OE, BIT(14) | BIT(13) | BIT(12), 0x1); /* Tx Main (HW control)(The right antenna) */

			/* 4 [ Rx ] */
			odm_set_bb_reg(p_dm, REG_FPGA0_XB_RF_INTERFACE_OE, BIT(5) | BIT(4) | BIT(3), 0x1); /* ant_div_type = TRDiv, right antenna */
			if (p_dm->ant_div_type == CGCS_RX_HW_ANTDIV)
				odm_set_bb_reg(p_dm, REG_CONFIG_RAM64X16, BIT(31), 0x1);				 /* RxCG, Default is RxCG. ant_div_type = 2RDiv, left antenna */
		} else {
			/* odm_set_bb_reg(p_dm, REG_FPGA0_XA_RF_INTERFACE_OE, BIT(8)|BIT9, 0x1);		  */ /* Tx Aux (SW control)(The left antenna) */
			/* 4 [ Tx ] */
			odm_set_bb_reg(p_dm, REG_FPGA0_XA_RF_INTERFACE_OE, BIT(14) | BIT(13) | BIT(12), 0x0);	 /* Tx Aux (HW control)(The left antenna) */

			/* 4 [ Rx ] */
			odm_set_bb_reg(p_dm, REG_FPGA0_XB_RF_INTERFACE_OE, BIT(5) | BIT(4) | BIT(3), 0x0); /* ant_div_type = TRDiv, left antenna */
			if (p_dm->ant_div_type == CGCS_RX_HW_ANTDIV)
				odm_set_bb_reg(p_dm, REG_CONFIG_RAM64X16, BIT(31), 0x0);				 /* RxCS, ant_div_type = 2RDiv, right antenna */
		}
	}
}
...
#else
		_phy_set_rf_path_switch_8192e(p_dm, is_main, false);
#endif
	}
}
