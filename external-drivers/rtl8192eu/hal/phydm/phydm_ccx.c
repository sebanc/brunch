...
void phydm_ifs_clm_get_utility(void *dm_void)
{
	...
	denomirator = ccx->ifs_clm_period;
	numerator = ccx->ifs_clm_tx * 100;
	ccx->ifs_clm_tx_ratio = (u8)PHYDM_DIV(numerator, denomirator);
	...
	ccx->ifs_clm_fa_ratio = (u8)PHYDM_DIV(numerator, denomirator);
	numerator = (ccx->ifs_clm_cckcca_excl_fa +
		     ccx->ifs_clm_ofdmcca_excl_fa) * 100;
	ccx->ifs_clm_cca_excl_fa_ratio = (u8)PHYDM_DIV(numerator, denomirator);
		       ...
		       u16 period, u8 ctrl_unit, s16 th_shift)
{
	...
	if (phydm_ifs_clm_th_update_chk(dm, ccx->ifs_clm_app, &ifs_clm_th_en[0],
...
u8 phydm_ifs_clm_mntr_set(void *dm_void, struct ifs_clm_para_info *ifs_clm_para)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
		       ...
		       char *output, u32 *_out_len)
{
	...
	if ((strcmp(input[1], help) == 0)) {
		...
		PDM_SNPF(out_len, used, output + used, out_len - used,
			 "IFS_CLM Adv-Trigger: {2} {App} {LV:1~4} {0~2096ms} {th_shift}\n");
		PDM_SNPF(out_len, used, output + used, out_len - used,
			      ...
			      struct enhance_mntr_trig_rpt *trig_rpt)
{
	...
	PHYDM_DBG(dm, DBG_ENV_MNTR, "ifs_clm_rpt_stamp=%d\n\n",
			    ...
			    char *output, u32 *_out_len)
{
	...
	if ((strcmp(input[1], help) == 0)) {
		PDM_SNPF(out_len, used, output + used, out_len - used,
			 "Basic-Trigger 960ms: {1}\n");
		PDM_SNPF(out_len, used, output + used, out_len - used,
			 "Get Result: {100}\n");
	} else if (var1[0] == 100) { /* Get results */
		phydm_enhance_mntr_result(dm, &rpt);
		...
		ifs_clm_para.ifs_clm_app = HP_TAS;
		ifs_clm_para.ifs_clm_lv = IFS_CLM_LV_2;
		ifs_clm_para.mntr_time = 960;
		ifs_clm_para.th_shift = 0;

		set_result = phydm_enhance_mntr_trigger(dm, &ifs_clm_para,
							&trig_rpt);

		PDM_SNPF(out_len, used, output + used, out_len - used,
			 "Set Result=%d, ifs_clm_rpt_stamp=%d\n",
			 set_result, trig_rpt.ifs_clm_rpt_stamp);
	}
